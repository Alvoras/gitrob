...
import (
	...
	"github.com/alvoras/gitrob/core"
)
...
func GatherRepositories(sess *core.Session) {
	...
	if len(sess.Targets) == 1 {
		...
		threadNum = (len(sess.Targets) + len(sess.Options.Repositories)) - 1
	} else {
	...
	for _, repoPath := range sess.Options.Repositories {
		repo, err := core.GetRepositoryFromPath(repoPath, sess.GithubClient)
		if err != nil {
			sess.Out.Error(" Failed to retrieve repository %s: %s\n", repoPath, err)
			continue
		}
...
func main() {
	...
	if sess.Stats.Status == "finished" {
		...
		if len(sess.Options.Logins) == 0 && len(sess.Options.Repositories) == 0{
